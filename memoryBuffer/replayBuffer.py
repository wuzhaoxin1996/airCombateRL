    ...
    def store(self, **args):
        ...
        根据任务自己定义：
            如果是常规单智能体，子类中可以使用如下参数：
                state, action, reward, next_state, done, ep_return=0
            如果需要存储多智能体，则酌情改变参数，
...
class ReplayBufferTransition(Buffer):
    def __init__(self, capacity, flag_piexl=0):
        ...
        self.Transition = collections.namedtuple("Transition" , ["state", "action", "reward", "next_state", "done", "episode_return"])
    ...
    def store(self, state, action, reward, next_state, done, ep_return=0):
        ...
        self.replay_buffer.append(self.Transition(state, action , reward , next_state , float(done), ep_return))
        self._update_size()
